...
local function decode_vendor_bitfield(bs, start_offset, max_id, target_vendors)
    ...
    if target_vendors then
        local target_map = {}
        for _, id in ipairs(target_vendors) do target_map[id] = true end
        ...
        if is_range then
            ...
            if target_map then
                for id = start_id, end_id do
                    if target_map[id] then res[id] = true end
                end
            else
                for id = start_id, end_id do
                    res[id] = true
...
local function decode_vendor_section(self, start_offset)
    ...
    if not max_id then return nil, start_offset end
    
    local is_range = self.bs:read_bool()
    local res
    if not is_range then
        res = decode_vendor_bitfield(self.bs, self.bs:pos(), max_id, self.options.targetVendors)
    ...
    for _ = 1, num_restrictions do
        ...
        for _ = 1, num_entries do
            ...
            for vendor_id = start_id, end_id do
                if not res[vendor_id] then res[vendor_id] = {} end
                if not res[vendor_id][purpose_id] then res[vendor_id][purpose_id] = {} end
...
local function decode_bitstream_field(self, bits, offset, is_bool)
    if offset then self.bs:seek(offset) end
    ...
    if is_bool then return val == 1 end
    return val
end
...
local FIELDS = {
    ...
    useNonStandardStacks = function(self)
        return decode_bitstream_field(self, 1, OFFSETS.USE_NON_STANDARD_STACKS, true)
    end,
